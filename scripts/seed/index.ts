...
try {
  ...
  console.warn(`.env.local not found at ${ENV_PATH} — relying on existing process.env`);
}
...
import { fetchAnimeThemesData } from './animethemes.ts';
import { enrichWithAnilist } from './anilist.ts';
import { enrichWithSpotify } from './spotify.ts';
import { upsertArtists, upsertAnimes, upsertSongs } from './upsert.ts';
...
const elapsed = (start: number): string => `${((Date.now() - start) / 1000).toFixed(2)}s`;
...
const run = async (): Promise<void> => {
  ...
  try {
    ...
    console.log(`Stage 2 done in ${elapsed(stage2Start)} — ${anilistEnrichment.size} entries`);
  } catch (err) {
    console.warn(`Stage 2 failed — continuing without AniList enrichment. Reason: ${(err as Error).message}`);
  ...
  try {
    ...
    console.log(`Stage 3 done in ${elapsed(stage3Start)} — ${spotifyEnrichment.size} entries`);
  } catch (err) {
    console.warn(`Stage 3 failed — continuing without Spotify enrichment. Reason: ${(err as Error).message}`);
